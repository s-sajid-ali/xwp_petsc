...
typedef struct {
  ...
  PetscViewer hdf5_ref_index_viewer; /* viewer to read refractive index*/
...
int main(int argc,char **args)
{
  ...
  appctx.step_grid_x = 10e-9;
  appctx.step_grid_y = 10e-9;
  ...
  appctx.step_z  = prop_distance/appctx.prop_steps;

  appctx.rid_fac = 2*pi*(appctx.step_z/appctx.lambda)*(PETSC_i*delta-beta);
  ...
  /* - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
     Run the main loop.
     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -*/
  ierr = ms_loop(A,slice_rid,u, u_,H,appctx.prop_steps,&appctx);CHKERRQ(ierr);
...
PetscErrorCode ms_loop(Mat A, Vec slice_rid, Vec u, Vec u_, Vec H, PetscInt prop_steps,AppCtx* appctx){
  ...
  char            fname[sizeof "ms_slice_100_100.h5"];
  ...
  for(i=0; i<prop_steps; i++){
    ...
    sprintf(fname,"ms_slice_%d_%d.h5",(int) appctx->prop_steps,(int) i);
    ierr = PetscViewerHDF5Open(PETSC_COMM_WORLD,fname,
	FILE_MODE_READ,&appctx->hdf5_ref_index_viewer);
    CHKERRQ(ierr);
    ierr = PetscObjectSetName((PetscObject)slice_rid,"ref_index");CHKERRQ(ierr);
    ierr = VecLoad(slice_rid,appctx->hdf5_ref_index_viewer);CHKERRQ(ierr);

    ierr = VecScale(slice_rid,appctx->rid_fac);CHKERRQ(ierr);
