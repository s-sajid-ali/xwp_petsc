...
/*    
  PetscInt size_1,size_2;
  if (rank==0){
  ierr = VecGetLocalSize(x1, &size_1); CHKERRQ(ierr);
  ierr = VecGetLocalSize(x2, &size_2); CHKERRQ(ierr);
  ierr = PetscPrintf(PETSC_COMM_SELF,"sizes : %d %d \n",size_1,size_2);CHKERRQ(ierr);
	}
  */  
